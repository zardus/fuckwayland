...
`resolute-gnome-iso` and `noble-gnome-iso` flavors instead use Ubuntu's installer
with the 26.04 and 24.04 desktop ISOs, so installation claims can be checked against
an ordinary desktop install. The `nixos-sway` and `nixos-gnome` images are built
from NixOS configurations with `nix build`.

Each VM logs in automatically as user `test` on a **multi-head virtio-vga** whose monitors are plugged,
...
`x11-utils` and `x11-xserver-utils`. The two **ISO** prepared images carry what the Ubuntu installer
...
X11 section is told to `apt install`. What the six tools currently manage on each desktop, including where they have no backend at all, is written down per flavor under *What the six
...
no firmware at all, 8 snaps against the default 13; 24.04: 240 / 55 and the same story, *The default install* and *The second default install*, below).
...
| `# vmctl-base:` | a cloud image in `$VMIMAGES` | n/a | `vmctl build` |
| `# vmctl-iso:` (+ `-iso-sha256`, `-iso-url`) | a release ISO | n/a | `vm/build-iso-golden.sh` |
| `# vmctl-nix:` | a `nixosConfigurations` attribute | n/a | `vm/build-nixos-golden.sh` |
...
| `# vmctl-base-sha256:` | 64 hex digits | n/a | `scripts/ci-golden.sh` |
...
* `dm_*`, one per display manager: `dm_gdm` (`gdm3/` or `gdm/`, whichever directory the package
  created; `WaylandEnable=false` only for an Xorg session; the session NAME resolved off the image, `ubuntu.desktop` on Ubuntu, `gnome.desktop` on Fedora and Arch, never from the flavor's
...
* SDDM: `/etc/sddm.conf.d/` autologin of `test` into the **Plasma Wayland** session, `plasmawayland.desktop` on 5.27, `plasma.desktop` on 6 (the build picks whichever
...
* **`noble-kde-x11`** and **`resolute-kde-x11`** are the same images built with `DESKTOP=kde-x11`: the same packages, the
  ...
  `kwin_x11` is a plain X window manager on top of it. The pair is a controlled experiment, one Plasma, one session bus, one `org.kde.KWin`, two session types. SDDM's autologin
...
2. **Configure** (1.5 min). The installed disk is booted once and `vm/build-iso-image.sh`
   ...
   disk *is* the prepared VM image, a plain qcow2 with no backing file, unlike the
   cloud-image ones.
...
* screen lock `true`, screensaver idle activation `true`, `idle-delay` `300`,
  ...
  override. A test that needs a desktop awake for ten minutes now has to say so itself, which is the honest place for it, because a user's machine will not have been told either.
...
layout operations, `warandr --command`/`--save`, the hotkey, and the locked-screen messages, is byte-identical. **A claim measured on `resolute-gnome` has, for these five tools, been
...
1. **`wwmctl -b remove,maximized_vert,maximized_horz` removes only the horizontal half, and
   ...
   command leaves a window that was `200 150 900 600` at `200 32 900 1048`, full height, and from there `-b remove,maximized_vert`, `wdotool windowstate --remove MAXIMIZED_VERT`
...
* **`gnome-initial-setup` 46 has no `-upgrade-login` unit.** It ships only
  ...
  conditioned on the *same* `~/.config/gnome-initial-setup-done` marker. So deviation 8, "complete the first run, do not remove it", writes **one** marker here where 26.04 needed
...
* **The installer leaves cloud-init off differently.** On both releases the target's *first*
  ...
  (`datasource_list: [None]`) and that run writes `/etc/cloud/cloud-init.disabled` itself, so the file the installer is usually said to write is written by cloud-init, one boot
...
2. **`wdotool windowstate` honours only the last `--add`/`--remove`/`--toggle` on the line**, also identical here: `--add MAXIMIZED_VERT --add MAXIMIZED_HORZ` gives `66 150 1854 600`,
...
3. **The locked-screen message hides the "extension not installed" one, on the exact path a
   ...
   *from the bus*, behind the lock screen the shell disables every extension, ours included, but it can tell them apart from disk, which is what `install-bridge.sh --check`
...
mode: OFF` (with no window open at all that last line is `N/A`, which is the reported result and
not a defect).
...
2. the native display tool lists exactly `Virtual-1`, `-2`, `-3`, with **`Virtual-1` at (0,0)**, and *primary* where the desktop has the notion: GNOME's logical-monitor `primary` flag and
...
[docs/WMIRROR.md](../docs/WMIRROR.md#where-it-does-not-exist) is the complete operation.
...
environment (`env -i`). Every message below is verbatim. None of it is a rig defect, `selftest.sh` passes on every flavor whose prepared image has been built. On the **X11 flavors** what is
measured is the
passthrough: on a plain X11 session the tools hand over to the real `xdotool`/`wmctrl`/`xprop`/
`xrandr` (repo README, *X11*), all four of which the prepared images carry, so there they behave as the
originals do.
...
| `arch-hypr`, `arch-sway`, `fedora44-gnome`, `fedora44-sway`, `nixos-sway` | no smoke ran | n/a |, |
...
| `resolute-labwc` | `--reuse --phases windows,wm,input` after the root-menu Escape | n/a | 31 pass, 0 fail, 1 XFAIL |
| `resolute-budgie` | whole run, all phases | 35 pass, 10 fail | 33 pass, 4 fail |
| `resolute-budgie` | `--reuse` of every phase after the detection fix | n/a | 56 pass, 0 fail |
| `resolute-lxqt-wayland` | whole run, all phases | 61 pass, 7 fail | 59 pass, 1 fail, 1 XFAIL |
| `resolute-lxqt-wayland` | `--reuse` of every phase after the workspace patch | n/a | 57 pass, 0 fail, 1 XFAIL |
...
| `resolute-labwc`, `resolute-xfce-wayland`, `resolute-budgie`, `resolute-lxqt-wayland` | works (`wlr`) | works (`wlroots wm`) | works, X and native windows | the generic wlr backend: no move, resize, raise or lower; desktops over `ext_workspace_manager_v1` | works | works |
...
session out and in) and both of them pass every cell of this table, on 46 and on 50, `wwmctl -l/-d/-m`, every `wdotool` window command and `wxprop -id` on native windows, as
...
Everything that needs the window list, `wwmctl -l`, `wdotool getactivewindow` and the rest, exits non-zero with
...
The prepared images are stock desktops and deliberately do not carry the extension: install it in the
...
`wwmctl -m` prints `Name: KWin` (`Class: N/A`, `PID: N/A`); `wwmctl -l` lists KWin's windows, XWayland rows with their real X ids and native rows with the backend id KWin's uuid is minted
...
shell started is `/usr/bin/xdotool`. Output is the original's, byte for byte, `xdotool getdisplaygeometry` (`1920 1080`, per screen), `getactivewindow`, `search`,
...
`Virtual-1 connected 1920x1080+0+0 (normal left inverted right x axis y axis) 487mm x 274mm`, X's numbers, not the Wayland backends'). `wwmctl -m` prints `Name: Xfwm4`, `Class: xfwm4` and
xfwm4's real `PID:`; `wwmctl -l` lists the X windows, `0x01600003 -1 <hostname> xfce4-panel`, `0x01800017 -1 <hostname> Desktop` per monitor, and a GTK
...
'xdotool help' if you want a command list`, rc 1. That is the handover working, not a defect, but nothing may claim `windowstate` works on X11. The listings above come from one 3-head run
...
which is xdotool's own wording for "nothing is focused", not a backend failure, `getdisplaygeometry` (`5760 1080`) and the window commands answer normally on the same session.
...
`windowlower` on any window warn and do nothing. `/dev/uinput` on this golden is root-only, the udev rule that hands the seat user an ACL lives under `gnome/` but is not GNOME-specific
...
having been measured on the generic wlroots backend, so the row worth writing is the difference,
...
still where it was. That last one is a defect of ours that can be fixed within this backend:
...
`wxprop` take the sway backend there and never the generic one. labwc has no IPC socket at all, the package is `labwc`, `labnag` and `lab-sensible-terminal` [recon2/labwc §1]. What the floor
...
over the X11 connection for XWayland windows only, and that would leave a listing where half the
...
madison` and sources.debian.org's exact search), and **the window manager is part of the image**, none is packaged anywhere, so `arch-river` builds upstream's own reference WM, tinyrwm (C,
...
and hyprland but not muffin, so without it the Wayland session has no X11 connection at all.
...
run is 57 pass, 0 fail, 7 XFAIL in 666 s) the window-management interface works: `wwmctl -m` is
...
written. Input is `/dev/uinput` only and works. `wmirror` refuses with a compositor-specific error
...
**The X11 connection of `resolute-cinnamon-wayland` is not yet.** Cinnamon starts fully (`Cinnamon took
...
there cannot be one**: that session is gone from 26.04, measured from the archive's own debs, `ubuntu-session` 46.0-1ubuntu4 ships `/usr/share/xsessions/ubuntu-xorg.desktop` and 50.1-0ubuntu0.1
...
still holds the WM selection, and after four rounds the unit goes `failed (Result: protocol)`, `Start request repeated too quickly`. That OnFailure unit is `ExecStart=gsettings set
...
* **An ISO flavor's prepared image has no backing file.** `vmctl build`'s prepared images are overlays on a
...
* **A build VM that is killed mid-boot stops at the GRUB menu on the next one.** Ubuntu's GRUB
  ...
  costs the target its ssh host keys if it happens on the first boot, before they are on disk, after which sshd resets every connection and the only cure is to install again.
