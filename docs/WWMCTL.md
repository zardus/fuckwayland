<a id="wwmctl-design-contract"></a>

# wwmctl: design contract

`wwmctl` is a drop-in `wmctrl` clone for Wayland that lists and controls both native
Wayland and XWayland windows. It shares window backends with wdotool and uses only
the Python standard library. Its compatibility target is the installed wmctrl;
[Technical.md](Technical.md) explains the shared implementation.
...
The compositor manages both native and XWayland windows, while the XWayland server
provides X11 properties. wwmctl combines these sources so scripts can use real X11
window IDs where available.

sway and i3 expose X11 IDs in the tree's `window` field. Hyprland, Wayfire and
Cinnamon provide equivalent identifiers through their own interfaces. The generic
wlr and COSMIC backends instead use `wdotool/xid_match.py` to match compositor
windows against `_NET_CLIENT_LIST`. These backends can compare titles and
`app_id`/`WM_CLASS`, but have no PID or geometry to distinguish otherwise identical
windows. Ambiguous matches retain ID 0 rather than selecting an arbitrary X11 ID.
...
- **Native Wayland windows** get their compositor node id as the printed id (collision
  ...
  also check X11 matches first on `-i` lookups). class comes from app_id
...
- **Actions** (activate, close, move/resize, desktops, states) go through the
  compositor backend for both window types because it manages XWayland windows too.
  The X11 connection is for identity/properties, not for actions (wlroots' xwm honors some
  EWMH root messages, but the compositor route is strictly more reliable).
- Without an X server, native-window operations continue to use compositor data;
  X11-specific properties are unavailable, and class data comes from the backend.
- **Machine column rule** (both planes): WM_CLIENT_MACHINE when the X11 connection can
...
`wdotool/x11_mini.py` implements the standard-library X11 client used by wwmctl.
...
`-N/-I/-T <STR> -r <STR>` (in a UTF-8 environment, any UTF-8 locale, or `-u`, the legacy `WM_NAME`/`WM_ICON_NAME` is *deleted* rather than written as a lossy
...
and from reference dumps of the real binary, which the devshell and every prepared VM image
carry.
...
window's `_NET_WM_ICON`, X11 connection only) and `-r <WIN> -L` (the window's own `-l` row).
...
**Limitations of the compositor interface, not defects.**
...
- Full desktops: `vm/vmctl`, whose prepared VM images all carry the real `wmctrl`,
...
the compositor interface is the w11 bridge extension
...
* **Columns.** `-x`: `instance.class` from the X11 connection's `WM_CLASS` for
  ...
  where every client is local, every session with XWayland or Wayland clients, the two rules print the same bytes. The desktop column is the workspace
...
* **`-k`** sends `_NET_SHOWING_DESKTOP` to the X root, exactly as real
  ...
  mode) and it is the fallback, for a session with no X11 connection or an
...
* **Actions** otherwise go through the bridge, for XWayland and native
  ...
  index), `-b (add|remove|toggle),P1[,P2]` `SetState` per property, `fullscreen`, `maximized_vert`/`maximized_horz` (real per-axis
  ...
  `modal`, and for an **XWayland** window those go to the X11 connection
  ...
  the gravity names the point of the window the request positions, `1` NorthWest puts the frame's top-left at `X,Y`, `5` Center puts its
  ...
  **GNOME 50 applies no gravity at all** and keeps the top-left corner
  ...
  window on the active workspace and restores exactly those on `off`, Mutter's own mode is not scriptable). `-n N` is `SetNWorkspaces`: works
...
* **`-N`/`-I`/`-T`** set `WM_NAME`/`_NET_WM_NAME` (and the icon names) on
  XWayland windows over the X11 connection (Mutter re-reads them at once); on
...
On a Plasma Wayland session (5.27 and 6.6) the compositor interface is
...
* **`-d`.** The VP column is `_NET_DESKTOP_VIEWPORT` read from the X server
  ...
  single pair prints it against the current desktop only. With no X11 connection
...
* **`-b`.** Both axes of `add,maximized_vert,maximized_horz` land: the
  ...
  wmctrl sends, which reaches an XWayland window through KWin's X11
...
<a id="hyprland-wayfire-the-wlroots-floor-and-cosmic"></a>

## Hyprland, Wayfire, the generic wlroots backend and COSMIC
...
* **`-m`** on a session with no X11 connection prints the compositor's own window-manager name
  rather than the backend token. `wlroots wm` on the generic wlr backend, that is the string every
...
* **`-l`** lists X and native windows together everywhere, and on the generic backend and on COSMIC
  ...
  `wdotool/xid_match.py`, and the generic backend and COSMIC feed it title and a lowercased
  `app_id` against `WM_CLASS` and nothing else, no pid, no geometry, so a pair that
  nothing separates keeps id 0.
* **`-d`** works on the generic wlr backend wherever the compositor publishes
...
* **`-e`** cannot yet move or resize windows through the generic wlr or COSMIC
  backends. The wlr protocol lacks geometry and stacking requests; adding support
  requires X11 configuration requests for XWayland clients (route 5) or compositor
  changes for native clients (route 6). COSMIC support requires changes to
  `cosmic-comp`. Hyprland supports the operation after a tiled window is made
  floating, and Wayfire supports it for its normally floating windows. Applications
  may round the requested size to their resize increments: in the recorded foot
  test, a request for `300x200` produced `300x195`.
