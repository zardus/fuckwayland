# fuckwayland.spec -- the Fedora counterpart of debian/.
#
...
URL:            https://github.com/antoniobianchi333/fuckwayland
...
# off, and `dnf install fuckwayland` on a sway box drags in no toolkit.  This is
...
# two.  fwcommon/distro.py's fedora row is these same names, and
...
gnome-shell-extension-fuckwayland-bridge subpackage, which dnf installs
...
the session bus as org.fuckwayland.Bridge.  gnome-shell scans extension
...
by /etc/xdg/autostart/fuckwayland-enable-bridge.desktop.
...
%install
%pyproject_install
%pyproject_save_files fwcommon wdotool wwmctl wxprop wxrandr warandr wmirror

install -Dpm 0644 gnome/60-fuckwayland-uinput.rules \
    %{buildroot}%{_udevrulesdir}/60-fuckwayland-uinput.rules
install -Dpm 0644 gnome/modules-load-uinput.conf \
    %{buildroot}%{_modulesloaddir}/fuckwayland-uinput.conf
...
#
#  * the helper is %%{_libexecdir}/fuckwayland/enable-bridge, not
#    /usr/lib/fuckwayland/enable-bridge, because /usr/libexec is where Fedora
...
install -pm 0644 gnome/%{bridge_uuid}/extension.js \
                 ...
                 gnome/%{bridge_uuid}/org.fuckwayland.Bridge1.xml \
...
sed 's|^Exec=.*|Exec=%{_libexecdir}/%{name}/enable-bridge|' \
    ...
    > %{buildroot}%{_sysconfdir}/xdg/autostart/fuckwayland-enable-bridge.desktop
chmod 0644 %{buildroot}%{_sysconfdir}/xdg/autostart/fuckwayland-enable-bridge.desktop
...
install -pm 0644 gnome/%{overlap_uuid}/extension.js \
                 ...
                 gnome/%{overlap_uuid}/org.fuckwayland.Overlap1.xml \
...
%post
# The same four commands, in the same order, as debian/fuckwayland.postinst and
...
# rule file away -- the reasoning is debian/fuckwayland.postrm's: udev keeps
...
%{_bindir}/warandr
%{_bindir}/wmirror
%{_udevrulesdir}/60-fuckwayland-uinput.rules
%{_modulesloaddir}/fuckwayland-uinput.conf
...
%{_libexecdir}/%{name}/
%{_sysconfdir}/xdg/autostart/fuckwayland-enable-bridge.desktop
...
- Same payload as fuckwayland (0.4.0) in debian/changelog.
